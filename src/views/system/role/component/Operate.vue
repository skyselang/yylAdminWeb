<template>
  <el-row>
    <el-col :span="6">
      <el-button v-if="hasPerm(['admin/system.Role/add'])" type="primary" @click="add()">{{ $t('添加') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Role/edit'])" @click="edit()">{{ $t('修改') }}</el-button>
      <el-button v-else-if="hasPerm(['admin/system.Role/info'])" @click="edit()">{{ $t('信息') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Role/dele'])" @click="update('is_delete')">{{ $t('删除') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Role/disable'])" @click="update('is_disable')">
        {{ $t('禁用') }}
      </el-button>
    </el-col>
    <el-col :span="14">
      <template v-if="hasPerm(['admin/system.Role/update'])">
        <el-button :title="$t('修改编号')" @click="update('role_unique')">{{ $t('编号') }}</el-button>
        <el-button :title="$t('修改备注')" @click="update('remark')">{{ $t('备注') }}</el-button>
        <el-button :title="$t('修改排序')" @click="update('sort')">{{ $t('排序') }}</el-button>
        <el-button :title="$t('修改菜单')" @click="update('menu_ids')">{{ $t('菜单') }}</el-button>
      </template>
    </el-col>
    <el-col :span="4">
      <Exports v-if="hasPerm(['admin/system.Role/export'])" :name="name" :api="exportApi" :query="query" :ids="ids" />
      <Imports v-if="hasPerm(['admin/system.Role/import'])" :name="name" :api="importApi" />
    </el-col>
  </el-row>
  <el-dialog
    v-model="dialog"
  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-else-if="field === 'role_unique'" :label="$t('编号')">
        ...
        <el-form-item v-else-if="field === 'menu_ids'" :label="$t('菜单')">
          <Menus v-model="value" :data="basedata.menus" :ids="basedata.menu_ids" :height="350" />
        </el-form-item>
...
import { deleApi, disableApi, updateApi, exportApi, importApi } from '@/api/system/role'
