  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-else-if="field === 'menu_pid'" :label="$t('上级')">
          <el-cascader
            v-model="value"
        ...
        <el-form-item v-else-if="field === 'active_menu_id'" :label="$t('激活菜单')">
          <el-cascader
            v-model="value"
            :options="basedata.trees"
            :props="basedata.props"
        ...
        <el-form-item v-else-if="field === 'lift_role'" :label="$t('解除角色')">
          <el-text size="default">{{ $t('确定要解除选中的的角色吗？', { name: name }) }}</el-text>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      } else if (this.field === 'menu_pid') {
        api = editPidApi
        data = { ids: this.ids, menu_pid: this.value }
