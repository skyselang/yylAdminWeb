  ...
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
        ...
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('发件人')" prop="sender">
                <el-input v-model="model.sender" clearable />
              </el-form-item>
              <el-form-item :label="$t('收件人')" prop="recipient">
                <el-input v-model="model.recipient" clearable />
              </el-form-item>
              <el-form-item :label="$t('主题')" prop="theme">
                <el-input v-model="model.theme" />
              </el-form-item>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/system.EmailLog/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/system/email-log'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = {
        sender: [{ required: true, message: this.$t('请输入发件人'), trigger: 'blur' }],
        recipient: [{ required: true, message: this.$t('请输入收件人'), trigger: 'blur' }]
      }
      if (this.dialog) {
