  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-else-if="field === 'dept_ids'" :label="$t('部门')">
          <el-cascader v-model="value" :options="basedata.depts" :props="basedata.deptProps" clearable class="w-full" />
        </el-form-item>
        <el-form-item v-else-if="field === 'post_ids'" :label="$t('职位')">
          <el-cascader v-model="value" :options="basedata.posts" :props="basedata.postProps" clearable class="w-full" />
        </el-form-item>
        <el-form-item v-else-if="field === 'role_ids'" :label="$t('角色')">
          <el-select v-model="value" clearable multiple class="w-full">
        ...
        <el-form-item v-else-if="field === 'password'" :label="$t('密码')">
          <el-input v-model="value" :placeholder="$t('请输入', { name: $t('密码') })" clearable />
        </el-form-item>
        <el-form-item v-else-if="field === 'is_super'" :label="$t('超管')">
          <ElSwitchWhether v-model="value" />
        </el-form-item>
...
export default {
  ...
  props: {
    ...
    basedata: {
      type: Object,
      default() {
        return { depts: [], posts: [], roles: [], deptProps: {}, postProps: {} }
      }
    }
  },
  emits: ['list', 'add', 'edit'],
  ...
  methods: {
    ...
    submit() {
      ...
      } else if (this.field === 'role_ids') {
        api = editRoleApi
        data = { ids: this.ids, role_ids: this.value }
