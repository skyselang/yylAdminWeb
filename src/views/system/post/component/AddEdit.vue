  ...
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
        ...
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('上级')" prop="post_pid">
                <el-cascader
              ...
              <el-form-item :label="$t('编号')" prop="post_unique">
                <el-input v-model="model.post_unique" clearable />
              </el-form-item>
              <el-form-item :label="$t('名称')" prop="post_name">
                <el-input v-model="model.post_name" clearable />
              </el-form-item>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/system.Post/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/system/post'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = { post_name: [{ required: true, message: this.$t('请输入名称'), trigger: 'blur' }] }
      if (this.dialog) {
    ...
    data(data) {
      ...
      if (this.pid) {
        this.model.post_pid = Number(this.pid)
      }
