  ...
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
        ...
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('上级')" prop="dept_pid">
                <el-cascader
              ...
              <el-form-item :label="$t('编号')" prop="dept_unique">
                <el-input v-model="model.dept_unique" clearable />
              </el-form-item>
              <el-form-item :label="$t('名称')" prop="dept_name">
                <el-input v-model="model.dept_name" clearable />
              </el-form-item>
              <el-form-item :label="$t('简称')" prop="abbr">
                <el-input v-model="model.abbr" clearable />
              </el-form-item>
              ...
              <el-form-item :label="$t('地址')" prop="addr">
                <el-input v-model="model.addr" clearable />
              </el-form-item>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/system.Dept/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/system/dept'
...
export default {
  ...
  data() {
    return {
      ...
      basedata: {
        trees: [],
        props: { value: 'dept_id', label: 'dept_name', checkStrictly: true, emitPath: false }
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = { dept_name: [{ required: true, message: this.$t('请输入名称'), trigger: 'blur' }] }
      if (this.dialog) {
    ...
    data(data) {
      ...
      if (this.pid) {
        this.model.dept_pid = Number(this.pid)
      }
