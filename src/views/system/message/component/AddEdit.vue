<template>
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
    <el-tabs>
      ...
      <el-tab-pane :label="$t('用户信息')" lazy>
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('接收用户')" prop="user_ids">
                <el-select-v2
                ...
                >
                  <template #header>
                    <el-checkbox v-model="userCheckAll" :indeterminate="userIndeterminate" @change="userChange">
                      {{ $t('全选/反选') }}
                    </el-checkbox>
                  </template>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/system.Message/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
<script>
import { screenHeight, addEditSuccessAlert, hasPerm, arrayColumn } from '@/utils/index'
import { infoApi, addApi, editApi } from '@/api/system/message'
...
export default {
  ...
  watch: {
    'model.user_ids': function (val) {
      this.userCheckAll = val.length === this.basedata.users.length
      this.userIndeterminate = val.length > 0 && !this.userCheckAll
    }
  },
  created() {
    this.init()
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = {
        ...
        user_ids: [{ required: true, message: this.$t('请选择接收用户'), trigger: 'blur' }]
      }
      if (this.dialog) {
    ...
    userChange() {
      if (this.userCheckAll) {
        this.model.user_ids = arrayColumn(this.basedata.users, 'user_id')
      } else {
        this.model.user_ids = []
      }
    }
