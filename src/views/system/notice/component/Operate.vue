<template>
  <el-row>
    <el-col :span="6">
      <el-button v-if="hasPerm(['admin/system.Notice/add'])" type="primary" @click="add()">{{ $t('添加') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Notice/edit'])" @click="edit()">{{ $t('修改') }}</el-button>
      <el-button v-else-if="hasPerm(['admin/system.Notice/info'])" @click="edit()">{{ $t('信息') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Notice/dele'])" @click="update('is_delete')">{{ $t('删除') }}</el-button>
      <el-button v-if="hasPerm(['admin/system.Notice/disable'])" @click="update('is_disable')">
        {{ $t('禁用') }}
      </el-button>
    </el-col>
    <el-col :span="14">
      <template v-if="hasPerm(['admin/system.Notice/update'])">
        ...
        <el-button :title="$t('修改类型')" @click="update('type')">{{ $t('类型') }}</el-button>
    ...
    <el-col :span="4">
      <Exports v-if="hasPerm(['admin/system.Notice/export'])" :name="name" :api="exportApi" :query="query" :ids="ids" />
      <Imports v-if="hasPerm(['admin/system.Notice/import'])" :name="name" :api="importApi" />
    </el-col>
  </el-row>
  <el-dialog
    v-model="dialog"
  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-else-if="field === 'type'" :label="$t('类型')">
...
import { deleApi, disableApi, updateApi, exportApi, importApi } from '@/api/system/notice'
