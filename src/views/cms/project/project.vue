<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-row :gutter="0">
        <el-col :xs="24" :sm="20">
          <el-input v-model="query.project_id" class="filter-item" style="width: 110px;" placeholder="ID" clearable />
    ...
    <el-table v-loading="loading" :data="data" :height="height-50" style="width: 100%" border @sort-change="sort" @selection-change="select">
      ...
      <el-table-column prop="project_id" label="ID" min-width="80" sortable="custom" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialog" width="65%" top="1vh" :before-close="cancel" @opened="dialogOpened()" @closed="dialogClosed()" @close-on-click-modal="false">
      <el-form ref="ref" :rules="rules" :model="model" class="dialog-body" label-width="100px" :style="{height:height+'px'}">
        <el-form-item label="分类" prop="project_category_id" clearable placeholder="请选择">
          <el-cascader
        ...
        <el-form-item v-if="model.project_id" label="添加时间" prop="create_time">
          <el-input v-model="model.create_time" disabled />
        </el-form-item>
        <el-form-item v-if="model.project_id" label="修改时间" prop="update_time">
    ...
    <el-dialog :title="recoverDialogTitle" :visible.sync="recoverDialog" width="80%" top="1vh">
      <div class="filter-container">
        <el-row :gutter="0">
          <el-col :xs="24" :sm="24">
            <el-input v-model="recoverQuery.project_id" class="filter-item" style="width: 110px;" placeholder="ID" clearable />
            <el-input v-model="recoverQuery.name" class="filter-item" style="width: 200px;" placeholder="名称" clearable />
            <el-cascader
              v-model="recoverQuery.project_category_id"
              class="filter-item"
              :options="projectCategory"
              :props="{checkStrictly: true, value: 'project_category_id', label: 'category_name'}"
              style="width:150px"
...
import { list, category, info, add, edit, dele, istop, ishot, isrec, ishide, recover, recoverReco, recoverDele } from '@/api/project'
...
export default {
  ...
  data() {
    return {
      ...
      is_hide: 0,
      selection: [],
      uploadAction: process.env.VUE_APP_BASE_API + '/admin/Project/upload',
      uploadHeaders: { AdminToken: getAdminToken() },
      uploadData: { type: 'image' },
      uploadFileData: { type: 'file' },
      editor: null,
      rules: {
        project_category_id: [{ required: true, message: '请输入分类', trigger: 'blur' }],
  ...
  methods: {
    ...
    edit(row) {
      this.loading = true
      this.dialog = true
      this.dialogTitle = this.name + '修改：' + row.project_id
      info({
    ...
    reset(row) {
      if (row) {
        this.model = row
        if (this.model.project_category_id === 0) {
          this.model.project_category_id = ''
        }
      } else {
        this.model = this.$options.data().model
      }
    ...
    category() {
      category().then(res => {
        this.projectCategory = res.data.list
      }).catch(() => {})
    },
    // 父级选择
    pidChange(value) {
      if (value) {
        this.model.project_category_id = value[value.length - 1]
      }
    },
    // 父级选择（查询）
    pidChangeQuery(value) {
      if (value) {
        this.query.project_category_id = value[value.length - 1]
      }
    },
    // 上传图片
    ...
    recoverPidChangeQuery(value) {
      if (value) {
        this.recoverQuery.project_category_id = value[value.length - 1]
      }
    }
  }
}
