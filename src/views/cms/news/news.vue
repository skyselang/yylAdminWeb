  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-row :gutter="0">
        <el-col :xs="24" :sm="20">
          ...
          <el-cascader
            v-model="query.news_category_id"
            class="filter-item"
            :options="newsCategory"
            :props="{checkStrictly: true, value: 'news_category_id', label: 'category_name'}"
            style="width:150px"
            clearable
            filterable
            placeholder="分类"
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialog" width="65%" top="1vh" :before-close="cancel" @opened="dialogOpened()" @closed="dialogClosed()" @close-on-click-modal="false">
      <el-form ref="ref" :rules="rules" :model="model" class="dialog-body" label-width="100px" :style="{height:height+'px'}">
        <el-form-item label="分类" prop="news_category_id" clearable placeholder="请选择">
          <el-cascader
            v-model="model.news_category_id"
            :options="newsCategory"
            :props="{checkStrictly: true, value: 'news_category_id', label: 'category_name'}"
            style="width:100%"
            clearable
        ...
        <el-form-item v-if="model.news_id" label="修改时间" prop="update_time">
    ...
    <el-dialog :title="recoverDialogTitle" :visible.sync="recoverDialog" width="80%" top="1vh">
      <div class="filter-container">
        <el-row :gutter="0">
          <el-col :xs="24" :sm="24">
            <el-input v-model="recoverQuery.news_id" class="filter-item" style="width: 110px;" placeholder="ID" clearable />
            <el-input v-model="recoverQuery.name" class="filter-item" style="width: 200px;" placeholder="名称" clearable />
            <el-cascader
              v-model="recoverQuery.news_category_id"
              class="filter-item"
              :options="newsCategory"
              :props="{checkStrictly: true, value: 'news_category_id', label: 'category_name'}"
              style="width:150px"
...
import { list, category, info, add, edit, dele, istop, ishot, isrec, ishide, recover, recoverReco, recoverDele } from '@/api/news'
...
export default {
  ...
  data() {
    return {
      ...
      editor: null,
      rules: {
        news_category_id: [{ required: true, message: '请输入分类', trigger: 'blur' }],
  ...
  methods: {
    ...
    edit(row) {
      this.loading = true
      this.dialog = true
      this.dialogTitle = this.name + '修改：' + row.news_id
      info({
        news_id: row.news_id
      }).then(res => {
        this.reset(res.data)
    ...
    pidChange(value) {
      if (value) {
        this.model.news_category_id = value[value.length - 1]
      }
    },
    // 父级选择（搜索）
    pidChangeQuery(value) {
      if (value) {
        this.query.news_category_id = value[value.length - 1]
      }
    },
    // 上传图片
    ...
    recoverPidChangeQuery(value) {
      if (value) {
        this.recoverQuery.news_category_id = value[value.length - 1]
      }
    }
  }
}
