<template>
  <div class="app-container">
    ...
    <!-- 列表 -->
    <el-table ref="table" v-loading="loading" :data="data" :height="height" style="width: 100%" row-key="product_category_id" default-expand-all border @selection-change="select">
      <el-table-column type="selection" width="40" />
      <el-table-column prop="category_name" label="分类名称" min-width="250" show-overflow-tooltip />
      <el-table-column prop="product_category_id" label="分类ID" min-width="80" />
      <el-table-column prop="product_category_pid" label="PID" min-width="80" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialog" top="1vh" width="65%" :before-close="cancel" :close-on-click-modal="false">
      <el-form ref="ref" :rules="rules" :model="model" class="dialog-body" label-width="100px" :style="{height:height+'px'}">
        <el-form-item label="分类父级" prop="product_category_pid">
          <el-cascader
            v-model="model.product_category_pid"
            :options="data"
        ...
        <el-form-item v-if="model.product_category_id" label="添加时间" prop="create_time">
          <el-input v-model="model.create_time" disabled />
        </el-form-item>
        <el-form-item v-if="model.product_category_id" label="修改时间" prop="update_time">
...
import { list, info, add, edit, dele, ishide } from '@/api/product-category'
...
export default {
  ...
  data() {
    return {
      ...
      is_hide: 0,
      selection: [],
      uploadAction: process.env.VUE_APP_BASE_API + '/admin/ProductCategory/upload',
  ...
  methods: {
    ...
    add(row) {
      ...
      if (row) {
        this.model.product_category_pid = row.product_category_id
      }
    },
    // 修改
    edit(row) {
      this.loading = true
      this.dialog = true
      this.dialogTitle = this.name + '修改：' + row.product_category_id
      info({
    ...
    dele(row) {
      if (row.length === 0) {
        ...
        if (row.length === 1) {
          title = title + '：' + row[0].product_category_id
    ...
    submit() {
      this.$refs['ref'].validate(valid => {
        if (valid) {
          this.loading = true
          if (this.model.product_category_id) {
    ...
    pidChange(value) {
      if (value) {
        this.model.product_category_pid = value[value.length - 1]
      }
    },
    // 上传图片
