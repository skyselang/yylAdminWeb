  ...
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
          ...
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('编号')" prop="tag_unique">
                <el-input v-model="model.tag_unique" clearable />
              </el-form-item>
              <el-form-item :label="$t('名称')" prop="tag_name">
                <el-input v-model="model.tag_name" clearable />
              </el-form-item>
              <el-form-item :label="$t('描述')" prop="tag_desc">
                <el-input v-model="model.tag_desc" />
              </el-form-item>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/file.Tag/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/file/tag'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = { tag_name: [{ required: true, message: this.$t('请输入名称'), trigger: 'blur' }] }
      if (this.dialog) {
