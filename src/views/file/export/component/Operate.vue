  ...
  <el-row>
    <el-col :span="6">
      <el-scrollbar-height>
        ...
        <el-button v-if="hasPerm(['admin/file.Export/dele']) && !query.recycle" @click="update('is_delete')">
          {{ $t('删除') }}
        </el-button>
        <el-button v-if="hasPerm(['admin/file.Export/recycleDele']) && query.recycle" @click="update('is_delete')">
          {{ $t('删除') }}
        </el-button>
