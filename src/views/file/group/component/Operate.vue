<template>
  <el-row>
    <el-col :span="6">
      <el-button v-if="hasPerm(['admin/file.Group/add'])" type="primary" @click="add()">{{ $t('添加') }}</el-button>
      <el-button v-if="hasPerm(['admin/file.Group/edit'])" @click="edit()">{{ $t('修改') }}</el-button>
      <el-button v-else-if="hasPerm(['admin/file.Group/info'])" @click="edit()">{{ $t('信息') }}</el-button>
      <el-button v-if="hasPerm(['admin/file.Group/dele'])" @click="update('is_delete')">{{ $t('删除') }}</el-button>
      <el-button v-if="hasPerm(['admin/file.Group/disable'])" @click="update('is_disable')">{{ $t('禁用') }}</el-button>
    </el-col>
    <el-col :span="14">
      <template v-if="hasPerm(['admin/file.Group/update'])">
    ...
    <el-col :span="4">
      <Exports v-if="hasPerm(['admin/file.Group/export'])" :name="name" :api="exportApi" :query="query" :ids="ids" />
      <Imports v-if="hasPerm(['admin/file.Group/import'])" :name="name" :api="importApi" />
    </el-col>
  </el-row>
  <el-dialog
    v-model="dialog"
...
import { deleApi, disableApi, updateApi, exportApi, importApi } from '@/api/file/group'
