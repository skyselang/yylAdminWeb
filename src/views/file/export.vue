  ...
  <div class="app-container">
    ...
    <el-row>
      <el-col class="mb-2">
        ...
        <el-button
          v-if="checkPermission(['admin/file.Export/recycleList'])"
    ...
    >
      ...
      <el-table-column prop="file_name" label="名称" min-width="150" sortable="custom" show-overflow-tooltip />
      ...
      <el-table-column prop="remark" label="备注" min-width="90" sortable="custom" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="160">
        <template #default="scope">
          <el-link type="primary" class="mr-1" :underline="false" :title="scope.row.file_path" @click="down(scope.row)">
            下载
          </el-link>
        ...
        <el-form ref="ref" :rules="rules" :model="model" label-width="100px">
          ...
          <el-form-item label="文件链接" prop="file_url">
            <el-input v-model="model.file_url" disabled>
              <template #append>
                <el-button title="复制" @click="clipboard(model.file_url)">
...
import { list, info, edit, dele, recycleList, recycleReco, recycleDele } from '@/api/file/export'
...
export default {
  ...
  methods: {
    ...
    down(file) {
      if (file.status != 3) {
        ElMessage.error('未处理完成无法下载')
      } else {
        info({ [this.idkey]: file[this.idkey], file_name: file.file_name, is_down: 1 }, true)
      }
    }
  }
}
