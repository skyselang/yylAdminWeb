...
<script>
import { hasPerm, screenHeight, getPageLimit } from '@/utils/index'
  ...
  data() {
    return {
      ...
      query: {
        ...
        search_mode: 'and',
        search: [{ field: 'file_name', exp: 'like', value: '' }],
        recycle: false,
        table: false,
        tag_ids: [],
        group_id: '',
  ...
  methods: {
    ...
    searchField() {
      this.fields = [
        ...
        { field: 'file_name', name: this.$t('名称'), exp: 'like' },
        { field: 'add_type', name: this.$t('新增方式'), exp: '=' },
        { field: 'domain', name: this.$t('域名'), exp: '=' },
        { field: 'file_ext', name: this.$t('后缀'), exp: '=' },
        { field: 'file_path', name: this.$t('路径'), exp: 'like' },
        { field: 'file_md5', name: 'MD5', exp: 'like' },
        { field: 'file_hash', name: this.$t('散列'), exp: 'like' },
    ...
    recycle() {
      this.query.recycle = !this.query.recycle
      this.$emit('search', this.query)
    },
    table() {
      this.query.table = !this.query.table
      this.$emit('search', this.query)
    }
  }
}
