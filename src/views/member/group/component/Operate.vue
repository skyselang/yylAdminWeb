<template>
  <el-row>
    <el-col :span="6">
      <el-button v-if="hasPerm(['admin/member.Group/add'])" type="primary" @click="add()">{{ $t('添加') }}</el-button>
      <el-button v-if="hasPerm(['admin/member.Group/edit'])" @click="edit()">{{ $t('修改') }}</el-button>
      <el-button v-else-if="hasPerm(['admin/member.Group/info'])" @click="edit()">{{ $t('信息') }}</el-button>
      <el-button v-if="hasPerm(['admin/member.Group/dele'])" @click="update('is_delete')">{{ $t('删除') }}</el-button>
      <el-button v-if="hasPerm(['admin/member.Group/disable'])" @click="update('is_disable')">
        {{ $t('禁用') }}
      </el-button>
    </el-col>
    <el-col :span="14">
      <template v-if="hasPerm(['admin/member.Group/update'])">
        ...
        <el-button :title="$t('修改默认')" @click="update('is_default')">{{ $t('默认') }}</el-button>
        <el-button :title="$t('修改接口')" @click="update('api_ids')">{{ $t('接口') }}</el-button>
      </template>
    </el-col>
    <el-col :span="4">
      <Exports v-if="hasPerm(['admin/member.Group/export'])" :name="name" :api="exportApi" :query="query" :ids="ids" />
      <Imports v-if="hasPerm(['admin/member.Group/import'])" :name="name" :api="importApi" />
    </el-col>
  </el-row>
  <el-dialog
    v-model="dialog"
  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-else-if="field === 'is_default'" :label="$t('默认')">
          <ElSwitchWhether v-model="value" />
        </el-form-item>
        <el-form-item v-else-if="field === 'api_ids'" :label="$t('接口')">
          <Api v-model="value" :data="basedata.apis" :ids="basedata.api_ids" :height="350" />
        </el-form-item>
...
import { deleApi, disableApi, updateApi, exportApi, importApi } from '@/api/member/group'
