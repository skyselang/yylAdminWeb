<template>
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
    <el-tabs>
          ...
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('编号')" prop="group_unique">
                <el-input v-model="model.group_unique" clearable />
              </el-form-item>
              <el-form-item :label="$t('名称')" prop="group_name">
                <el-input v-model="model.group_name" clearable />
              </el-form-item>
              <el-form-item :label="$t('描述')" prop="desc">
                <el-input v-model="model.desc" type="textarea" />
              </el-form-item>
              <el-form-item :label="$t('默认')" prop="is_default">
                <ElSwitchWhether v-model="model.is_default" />
              </el-form-item>
      ...
      <el-tab-pane :label="$t('接口信息')" lazy>
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('接口')">
                <Api v-model="model.api_ids" :data="basedata.apis" :ids="basedata.api_ids" :height="height - 100" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-scrollbar>
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/member.Group/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/member/group'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = { group_name: [{ required: true, message: this.$t('请输入名称'), trigger: 'blur' }] }
      if (this.dialog) {
