<template>
  ...
  >
    ...
    <el-row>
      <el-col :span="2">
        <el-button v-if="hasPerm(['admin/member.Api/groupLift'])" type="primary" @click="lift()">
          {{ $t('解除') }}
        </el-button>
      </el-col>
      <el-col :span="22">
    ...
    >
      ...
      <el-table-column :label="$t('操作')" min-width="80">
        <template #default="{ row }">
          <ElLinkOperate v-if="hasPerm(['admin/member.Api/groupLift'])" :text="$t('解除')" @click="lift(row)" />
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
...
export default {
  ...
  data() {
    return {
      ...
      data: [],
      count: 0,
      query: { page: 1, limit: getPageLimit(), search: [{ field: 'group_name', exp: 'like', value: '' }] },
      ids: [],
      selectDialog: false,
      basedata: {
        exps: [],
