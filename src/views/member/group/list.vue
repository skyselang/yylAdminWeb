<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="group_unique" :label="$t('编号')" min-width="120" sortable="custom" />
      <el-table-column prop="group_name" :label="$t('名称')" min-width="130" sortable="custom" />
      ...
      <el-table-column :label="$t('操作')" width="170">
        <template #default="{ row }">
          ...
          <ElLinkOperate v-if="hasPerm([permMember])" :text="$t('会员')" @click="member(row)" />
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    ...
    <Member
      ...
      title-pk="group_name"
      :title="group_name"
      :list-api="memberListApi"
      :lift-api="memberLiftApi"
      @close="member"
    />
  </div>
</template>
...
import { listApi, deleApi, memberListApi, memberLiftApi } from '@/api/member/group'
import Member from '@/views/member/member/component/Member.vue'

export default {
  name: 'MemberGroupList',
  components: { Search, Operate, AddEdit, Member },
  data() {
    return {
      name: '',
      height: 700,
      ...
      dialog: false,
      dialogTitle: '',
      basedata: { exps: [], apis: [], api_ids: [] },
      permInfo: 'admin/member.Group/info',
      permEdit: 'admin/member.Group/edit',
      permDele: 'admin/member.Group/dele',
      permMember: 'admin/member.Group/memberList',
      routeInfo: 'MemberGroupInfo',
      routeAdd: 'MemberGroupAdd',
      routeEdit: 'MemberGroupEdit',
      group_id: 0,
      group_name: ''
    }
  },
    ...
    list() {
      ...
      listApi(this.query)
        .then((res) => {
          ...
          this.basedata.apis = res.data.basedata.apis
          this.basedata.api_ids = res.data.basedata.api_ids
        })
        .catch(() => {
          this.loading = false
        })
