<template>
  <el-form ref="ref" :model="model" :rules="rules" label-width="130px" :class="{ 'app-container': formclass }">
    <el-tabs>
        ...
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              ...
              <el-form-item :label="$t('文件')" prop="file_id">
                <FileUpload v-model="model.file_id" :preview="model" />
              </el-form-item>
              <el-form-item :label="$t('标题')" prop="title">
                <el-input v-model="model.title" clearable />
              </el-form-item>
              <el-form-item :label="$t('位置')" prop="position">
                <el-input v-model="model.position" clearable />
              </el-form-item>
      ...
      <el-tab-pane :label="$t('文件信息')" lazy>
        <el-scrollbar :height="height">
          <el-row>
            <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
              <el-form-item :label="$t('文件列表')" prop="file_list">
                <FileUploads
    ...
    <el-form-item>
      <el-col :md="rowcol.md" :lg="rowcol.lg" :xl="rowcol.xl">
        ...
        <el-button
          v-if="id && type === 'edit' && hasPerm(['admin/setting.Carousel/add'])"
          :loading="loading"
          :title="$t('修改将会变成添加')"
          class="float-right"
          @click="submit('add')"
...
import { infoApi, addApi, editApi } from '@/api/setting/carousel'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.rules = { title: [{ required: true, message: this.$t('请输入标题'), trigger: 'blur' }] }
      if (this.dialog) {
