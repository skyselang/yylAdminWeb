<template>
  ...
  >
    ...
    <el-row>
      <el-col :span="2">
        <el-button
          v-if="hasPerm(['admin/content.Category/contentLift', 'admin/content.Tag/contentLift'])"
          type="primary"
          @click="lift()"
        >
          {{ $t('解除') }}
    ...
    >
      ...
      <el-table-column prop="name" :label="$t('名称')" min-width="175" />
      <el-table-column prop="category_names" :label="$t('分类')" min-width="105" />
      ...
      <el-table-column prop="is_rec_name" :label="$t('推荐')" min-width="96" sortable="custom" column-key="is_rec" />
      <el-table-column prop="hits" :label="$t('点击')" min-width="86" sortable="custom" />
...
export default {
  ...
  data() {
    return {
      ...
      data: [],
      count: 0,
      query: { page: 1, limit: getPageLimit(), search: [{ field: 'name', exp: 'like', value: '' }] },
      ids: [],
      selectDialog: false,
      basedata: {
        exps: [],
    ...
    list() {
      ...
      this.listApi(this.query)
        .then((res) => {
          ...
          this.basedata.categorys = res.data.basedata.categorys
