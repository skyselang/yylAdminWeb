<template>
  <el-row>
    <el-col :span="6">
      <el-button v-if="hasPerm(['admin/content.Content/add'])" type="primary" @click="add()">
        {{ $t('添加') }}
      </el-button>
      <el-button v-if="hasPerm(['admin/content.Content/edit'])" @click="edit()">{{ $t('修改') }}</el-button>
      <el-button v-else-if="hasPerm(['admin/content.Content/info'])" @click="edit()">{{ $t('信息') }}</el-button>
      <el-button v-if="hasPerm(['admin/content.Content/dele'])" @click="update('is_delete')">
        {{ $t('删除') }}
      </el-button>
      <el-button v-if="hasPerm(['admin/content.Content/disable'])" @click="update('is_disable')">
        {{ $t('禁用') }}
      </el-button>
    </el-col>
    <el-col :span="14">
      <template v-if="hasPerm(['admin/content.Content/update'])">
        ...
        <el-button :title="$t('修改图片')" @click="update('image_id')">{{ $t('图片') }}</el-button>
        <el-button :title="$t('修改分类')" @click="update('category_ids')">{{ $t('分类') }}</el-button>
        <el-button :title="$t('修改标签')" @click="update('tag_ids')">{{ $t('标签') }}</el-button>
        <el-button :title="$t('修改置顶')" @click="update('is_top')">{{ $t('置顶') }}</el-button>
        <el-button :title="$t('修改热门')" @click="update('is_hot')">{{ $t('热门') }}</el-button>
        <el-button :title="$t('修改推荐')" @click="update('is_rec')">{{ $t('推荐') }}</el-button>
        <el-button :title="$t('修改发布时间')" @click="update('release_time')">{{ $t('发布时间') }}</el-button>
        <el-button :title="$t('修改初始点击')" @click="update('hits_initial')">{{ $t('初始点击') }}</el-button>
      </template>
    </el-col>
    <el-col :span="4">
      <Exports
        v-if="hasPerm(['admin/content.Content/export'])"
        :name="name"
        :api="exportApi"
        :query="query"
        :ids="ids"
      />
      <Imports v-if="hasPerm(['admin/content.Content/import'])" :name="name" :api="importApi" />
    </el-col>
  </el-row>
  <el-dialog
    v-model="dialog"
  ...
  >
    <el-scrollbar :height="height">
      <el-form ref="ref" label-width="120px">
        ...
        <el-form-item v-if="field === 'category_ids'" :label="$t('分类')">
          <el-cascader
            v-model="value"
        ...
        <el-form-item v-else-if="field === 'image_id'" :label="$t('图片')">
          <el-col><el-input v-model="value" :placeholder="$t('请输入文件ID')" clearable /></el-col>
        </el-form-item>
        <el-form-item v-else-if="field === 'tag_ids'" :label="$t('标签')">
          <el-select v-model="value" clearable filterable multiple class="w-full">
            <el-option v-for="v in basedata.tags" :key="v.tag_id" :label="v.tag_name" :value="v.tag_id" />
          </el-select>
        </el-form-item>
        <el-form-item v-else-if="field === 'is_top'" :label="$t('置顶')">
          <ElSwitchWhether v-model="value" />
        </el-form-item>
        <el-form-item v-else-if="field === 'is_hot'" :label="$t('热门')">
          <ElSwitchWhether v-model="value" />
        </el-form-item>
        <el-form-item v-else-if="field === 'is_rec'" :label="$t('推荐')">
          <ElSwitchWhether v-model="value" />
        </el-form-item>
        <el-form-item v-else-if="field === 'release_time'" :label="$t('发布时间')">
          <ElDatePickerDatetime v-model="value" />
        </el-form-item>
        <el-form-item v-else-if="field === 'hits_initial'" :label="$t('初始点击')">
          <el-input v-model="value" type="number" clearable />
        </el-form-item>
...
import { deleApi, disableApi, updateApi, exportApi, importApi } from '@/api/content/content'
